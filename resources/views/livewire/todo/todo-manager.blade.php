<div class="animate-in" x-data="{ editingListId: null }">
    ...
    <div class="flex-header">
        <div>
            <h2 class="responsive-title">{{ __('Todo') }}</h2>
            <p style="color: var(--text-muted); font-size: 14px; margin-top: var(--space-1);">{{ __('Organize your tasks and stay productive') }}</p>
        </div>
        <div>
            @if($activeTodoId)
            ...
            @if($activeTodoId)
                <div style="display: flex; align-items: center; gap: 8px;" x-data="{ activeName: '{{ addslashes($this->activeTodo->name) }}' }" wire:key="todo-list-name-{{ $activeTodoId }}">
                    <div style="display: inline-grid; align-items: center; min-width: 120px; position: relative;">
                        ...
                        <span x-text="activeName || '{{ __('List name...') }}'" style="grid-area: 1/1; visibility: hidden; padding: 0 16px; white-space: pre; font-weight: 800; font-size: 14px; min-width: 120px;"></span>
                        <input 
                            type="text" 
                            x-model="activeName"
                    ...
                    <button 
                        wire:confirm="{{ __('Are you sure you want to delete this list and all its tasks?') }}"
        ...
        <div style="display: flex; align-items: center; gap: 8px; margin-left: auto;">
            ...
            <div style="position: relative;" x-data="{ open: false }">
                ...
                <div x-show="open" @click.outside="open = false" class="todo-filter-popout" style="top: calc(100% + 8px); background: var(--bg-card); border: 1px solid var(--border-color); border-radius: 16px; padding: 12px; min-width: 200px; box-shadow: var(--shadow-lg);" x-transition x-cloak>
                    <div style="font-size: 11px; font-weight: 800; color: var(--text-muted); text-transform: uppercase; margin-bottom: 12px; padding-bottom: 8px; border-bottom: 1px solid var(--border-color);">{{ __('Filter by Tag') }}</div>
                    ...
                    @if(count($selectedTags) > 0)
                        <div style="margin-top: 16px; padding-top: 12px; border-top: 1px solid var(--border-color);">
                            <button wire:click="$set('selectedTags', [])" style="width: 100%; padding: 8px; border: none; background: transparent; color: var(--danger); font-size: 12px; font-weight: 800; cursor: pointer;">{{ __('Clear Filters') }}</button>
                        </div>
                    @endif
                </div>
            </div>
            ...
            @if($this->pendingItems->isEmpty())
                <div class="card" style="border-radius: 28px; overflow: hidden; margin-bottom: 24px;">
                    <div style="padding: 40px; text-align: center; color: var(--text-muted); font-size: 14px; font-weight: 500;">
                        {{ __('No pending tasks in this list.') }}
                    </div>
                </div>
            @else
                        ...
                        <div class="card" style="border-radius: 28px; overflow: hidden; margin-bottom: 8px;">
                            @if(!$headerShown)
                                <div class="todo-grid-header">
                                    <div class="text-muted" style="text-align: center; font-size: 9px; font-weight: 800; text-transform: uppercase;">{{ __('Sort') }}</div>
                                    <div class="text-muted" style="text-align: center; font-size: 9px; font-weight: 800; text-transform: uppercase;">{{ __('Done') }}</div>
                                    <div class="text-muted" style="font-size: 9px; font-weight: 800; text-transform: uppercase; padding-left: 10px;">{{ __('Task Description') }}</div>
                                    <div class="text-muted" style="text-align: center; font-size: 9px; font-weight: 800; text-transform: uppercase;">{{ __('Del') }}</div>
                                </div>
                            ...
                            <div class="cell-name" style="display: flex; flex-direction: column; justify-content: center;">
                                ...
                                @if($item->completed_at)
                                    <span style="font-size: 11px; color: var(--text-muted); margin-top: 4px; opacity: 0.7; font-weight: 600;">
                                        {{ __("Completed:") }} {{ $item->completed_at->format('M j, Y') }}
                                    </span>
                                @endif
                            </div>
    ...
    @else
        ...
        <div style="text-align: center; padding: 100px 0;">
            ...
            <h2 style="font-size: 2rem; font-weight: 900; margin-bottom: 8px; letter-spacing: -0.02em;">{{ __('No lists found') }}</h2>
            <p style="color: var(--text-muted); font-size: 16px; margin-bottom: 40px; font-weight: 500;">{{ __('Create your first todo list to start organizing tasks.') }}</p>
