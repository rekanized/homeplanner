<div class="animate-in" style="max-width: 1280px; margin: 0 auto; padding: 40px 24px;">
    <div class="flex-header" style="align-items: flex-end;">
        <div>
            ...
            <p style="color: var(--text-muted); font-weight: 600; font-size: 14px; text-transform: uppercase; letter-spacing: 0.05em;">{{ __('Household member registration log') }}</p>
        </div>
        <div style="display: flex; align-items: center; gap: 16px; flex-wrap: wrap;">
            ...
            <div style="background: var(--bg-card); padding: 12px 20px; border-radius: 16px; border: 1px solid var(--border-color); box-shadow: var(--shadow-sm);">
                <span style="font-size: 11px; font-weight: 800; color: var(--text-muted); text-transform: uppercase;">{{ __('Total Members:') }}</span>
        ...
        <table class="responsive-table" style="width: 100%; border-collapse: collapse;">
            <thead>
                <tr style="background: var(--bg-input); border-bottom: 2px solid var(--border-color);">
                    ...
                    <th style="padding: 16px 24px; text-align: left; font-size: 11px; font-weight: 800; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.1em;">{{ __('Method') }}</th>
                    <th style="padding: 16px 24px; text-align: left; font-size: 11px; font-weight: 800; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.1em;">{{ __('Joined Date') }}</th>
                    <th style="padding: 16px 24px; text-align: right; font-size: 11px; font-weight: 800; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.1em;">{{ __('Account Role') }}</th>
                </tr>
            </thead>
            <tbody style="background: var(--bg-card);">
                @foreach($users as $user)
                    <tr style="border-bottom: 1px solid var(--border-color); transition: background 0.2s ease;">
                        ...
                        <td data-label="{{ __('Method') }}" style="padding: 20px 24px;">
                            @if($user->google_id)
                            ...
                            <div style="display: flex; align-items: center; justify-content: flex-end; gap: 16px;">
                                <div style="display: flex; align-items: center; gap: 8px;">
                                    ...
                                    @else
                                        ...
                                        @if($user->is_child)
                                            <span style="display: inline-block; padding: 4px 12px; border-radius: 100px; background: #6366f120; color: #6366f1; font-size: 11px; font-weight: 800; text-transform: uppercase; border: 1px solid #6366f140;">{{ __('Child') }}</span>
                                        @else
                                            <span style="display: inline-block; padding: 4px 12px; border-radius: 100px; background: var(--success-soft); color: var(--success); font-size: 11px; font-weight: 800; text-transform: uppercase;">{{ __('Member') }}</span>
                                        @endif
                                    @endif
                                </div>
                                ...
                                @if($user->is_child)
                                <div style="display: flex; align-items: center; gap: 8px;">
                                    <span style="font-size: 9px; font-weight: 800; color: var(--text-muted); text-transform: uppercase;">{{ __('Goal:') }}</span>
                                    <input type="number" 
            ...
            <div class="modal-content" style="max-width: 450px;">
                ...
                <form wire:submit.prevent="createUser" style="padding: 24px 32px 32px 32px; display: flex; flex-direction: column; gap: 20px;">
                    ...
                    <div style="display: flex; gap: 12px; margin-top: 8px;">
                        <button type="button" @click="$wire.set('showCreateModal', false)" class="btn" style="flex: 1; background: var(--bg-input); color: var(--text-main); border: 1px solid var(--border-color);">{{ __('Cancel') }}</button>
                        <button type="submit" class="btn btn-primary" style="flex: 2;">{{ __('Create Member') }}</button>
                    </div>
                </form>
            </div>
        </div>
